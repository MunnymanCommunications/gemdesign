...
serve(async (req: Request)=>{
  ...
  try {
    ...
    const { data: userDocs, error: userDocsError } = await supabase.from('user_documents').select('filename, file_path, mime_type, extracted_text').eq('user_id', userId);
    if (userDocsError) {
      console.error('Error fetching user documents:', userDocsError);
    }
    // Get global AI documents with extracted text
    const { data: globalDocs, error: globalDocsError } = await supabase.from('global_ai_documents').select('filename, file_path, mime_type, extracted_text');
    if (globalDocsError) {
    ...
    // Build context from documents with proper truncation rules
    let documentsContext = '';
    
    // Global documents: always include full text without truncation
    if (globalDocs && globalDocs.length > 0) {
      documentsContext += '\n\nCompany Documents Available:\n';
      for (const doc of globalDocs) {
        documentsContext += `- ${doc.filename} (${doc.mime_type})\n`;
        if (doc.extracted_text && !doc.extracted_text.includes('endstream') && !doc.extracted_text.includes('xref')) {
          documentsContext += `  Content: ${doc.extracted_text}\n`;
        }
      }
    }

    // User documents: conditionally include content based on keyword matches
    if (userDocs && userDocs.length > 0) {
      ...
      for (const doc of userDocs) {
        ...
        if (doc.extracted_text && !doc.extracted_text.includes('endstream') && !doc.extracted_text.includes('xref')) {
          if (hasMatch) {
            const truncatedContent = doc.extracted_text.length > 90000 
    ...
    if (structuredData && structuredData.siteImages && structuredData.siteImages.length > 0) {
      userContent = [
        ...
        ...structuredData.siteImages.map((imageData: string)=>({
            type: 'image_url',
            image_url: {
              url: imageData
            }
    ...
    if (ASSISTANT_ID) {
      ...
      if (runStatus === 'completed') {
        ...
        if (assistantMessage && assistantMessage.content && assistantMessage.content.length > 0) {
          const responseText = assistantMessage.content.text.value;
          return new Response(JSON.stringify({
      ...
      if (conversationId) {
        ...
        if (priorityMatch) {
          const score = parseInt(priorityMatch, 10);
